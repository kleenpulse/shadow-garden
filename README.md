...
<!-- catalog:start -->
61 components across 4 categories:
...
- **Power-User Systems** (13) — command surfaces for operators: Sakura Tree, Command Palette, Physics Engine, Black Hole, Vortex Bloom, Flock Field, Smoke Field, Ripple Field, File Explorer, Umbral Orrery, Shadow Bloom, Parallax Rail, Elastic.

Components are tiered **free** (30) or **pro** (31). Pro source is gated server-side and never crosses to the client unless unlocked. The dev overrides (env `SHADOW_GARDEN_PRO=1`, cookie `sg_pro=1`) only apply in local development — in production the gate is entitlement-only.
<!-- catalog:end -->
