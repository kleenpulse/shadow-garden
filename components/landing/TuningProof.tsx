...
export default function TuningProof({ tuning }: { tuning: LandingData["tuning"] }) {
        ...
        {tuning.sliders.map((slider) => (
          <label key={slider.name} className="block">
            ...
            <input
              ...
              value={values[slider.name]}
              onChange={(event) =>
                setValues((prev) => ({ ...prev, [slider.name]: Number(event.target.value) }))
              }
            />
          </label>
        ))}
        <p className="font-sans text-xs leading-relaxed text-ink-mute">
          Every specimen in the catalog exposes its dials like this — tuned live, reflected in
          the props table, ready to copy.
        </p>
      </div>
    </div>
  );
