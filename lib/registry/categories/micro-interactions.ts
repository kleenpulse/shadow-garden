import type { ComponentEntry, Variant } from "../types";

/** lib/utils.ts ships with every entry whose source imports `cn`. Its own
 *  packages (clsx, tailwind-merge) are the host entry's to declare — the check
 *  walks into this file and finds them there (§V41). */
const UTILS_VARIANT: Variant = {
	lang: "ts",
	style: "tailwind",
	file: "lib/utils.ts",
	role: "util",
	label: "utils.ts",
};

/** Micro-interactions — hover, focus, and dialog motion primitives. */
export const microInteractions: ComponentEntry[] = [
	{
		slug: "pixel-transition",
		name: "PixelTransition",
		category: "Micro-interactions",
		tier: "free",
		description:
			"Reveal a second layer through a random pixelated dissolve on hover.",
		dependencies: ["gsap", "clsx", "tailwind-merge"],
		cookbook: ["Reveal", "Stepped animation", "Hover effect", "Crossfade"],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/pixel-transition/PixelTransition.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "gridSize",
				kind: "number",
				default: 7,
				min: 2,
				max: 20,
				step: 1,
				description: "Pixel grid resolution.",
			},
			{
				name: "pixelColor",
				kind: "color",
				default: "#a855f7",
				description: "Color of the dissolve pixels.",
			},
			{
				name: "animationStepDuration",
				kind: "number",
				default: 0.3,
				min: 0.1,
				max: 1,
				step: 0.05,
				unit: "s",
				description: "Duration of the dissolve.",
			},
			{
				name: "once",
				kind: "boolean",
				default: false,
				description: "Play only once instead of on every hover.",
			},
		],
	},
	{
		slug: "border-glow",
		name: "BorderGlow",
		category: "Micro-interactions",
		tier: "free",
		description: "A cursor-proximity gradient border with an outer glow.",
		dependencies: [],
		cookbook: ["Hover effect", "Interpolation / Tween"],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/border-glow/BorderGlow.tsx",
			},
		],
		props: [
			{
				name: "glowColor",
				kind: "color",
				default: "#a855f7",
				description: "Glow color.",
			},
			{
				name: "glowRadius",
				kind: "number",
				default: 12,
				min: 0,
				max: 40,
				step: 1,
				unit: "px",
				description: "Outer glow radius.",
			},
			{
				name: "glowIntensity",
				kind: "number",
				default: 1,
				min: 0,
				max: 2,
				step: 0.05,
				description: "Glow brightness.",
			},
			{
				name: "coneSpread",
				kind: "number",
				default: 35,
				min: 5,
				max: 50,
				step: 1,
				description: "Angular spread of the glow cone.",
			},
			{
				name: "edgeSensitivity",
				kind: "number",
				default: 30,
				min: 0,
				max: 80,
				step: 1,
				description: "How close to the edge the glow activates.",
			},
			{
				name: "borderRadius",
				kind: "number",
				default: 16,
				min: 0,
				max: 40,
				step: 1,
				unit: "px",
				description: "Corner radius.",
			},
			{
				name: "fillOpacity",
				kind: "number",
				default: 0.5,
				min: 0,
				max: 1,
				step: 0.05,
				description: "Border fill opacity.",
			},
			{
				name: "animated",
				kind: "boolean",
				default: false,
				description: "Play an intro sweep on mount.",
			},
		],
	},
	{
		slug: "spotlight-shell",
		name: "SpotlightShell",
		category: "Micro-interactions",
		tier: "free",
		description:
			"A card shell with 3D magnetic tilt, radial glow, and a shimmer sweep.",
		dependencies: ["motion", "clsx", "tailwind-merge"],
		cookbook: [
			"3D tilt / Flip",
			"Perspective",
			"Hover effect",
			"Skeleton / Shimmer",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/spotlight-shell/SpotlightShell.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "accentColor",
				kind: "color",
				default: "#a855f7",
				description: "Accent color for the glow and accent line.",
			},
		],
	},
	{
		slug: "animated-hamburger",
		name: "AnimatedHamburger",
		category: "Micro-interactions",
		tier: "free",
		description:
			"A three-bar hamburger that morphs to an X, driven by a toggle prop.",
		dependencies: ["motion"],
		cookbook: ["Morph", "Rotate", "Translate"],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/animated-hamburger/AnimatedHamburger.tsx",
			},
		],
		props: [
			{
				name: "toggle",
				kind: "boolean",
				default: false,
				description: "Toggle between the hamburger and the X.",
			},
		],
	},
	{
		slug: "morph-dialog",
		name: "MorphDialog",
		category: "Micro-interactions",
		tier: "pro",
		description:
			"A dialog that morphs open from its trigger via shared-layout animation, with elements that travel between card and panel.",
		dependencies: ["motion", "lucide-react", "clsx", "tailwind-merge"],
		cookbook: [
			"Shared element transition",
			"Layout animation",
			"Morph",
			"Origin-aware animation",
			"Continuity transition",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/morph-dialog/MorphDialog.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "stiffness",
				kind: "number",
				default: 320,
				min: 100,
				max: 600,
				step: 10,
				description: "Spring stiffness of the morph.",
			},
			{
				name: "damping",
				kind: "number",
				default: 34,
				min: 10,
				max: 60,
				step: 1,
				description: "Spring damping — lower rings longer.",
			},
			{
				name: "radius",
				kind: "number",
				default: 16,
				min: 0,
				max: 32,
				step: 1,
				unit: "px",
				description: "Panel corner radius.",
			},
			{
				name: "blurBackdrop",
				kind: "boolean",
				default: true,
				description: "Frosted blur on the backdrop scrim.",
			},
			{
				name: "autoFocus",
				kind: "boolean",
				default: true,
				description: "Focus the first focusable element on open.",
			},
		],
	},
	{
		slug: "grow-dialog",
		name: "GrowDialog",
		category: "Micro-interactions",
		tier: "free",
		description:
			"A dialog that grows out of the clicked element and shrinks back into it, entirely inside a portal — never behind page content.",
		dependencies: ["motion", "lucide-react", "clsx", "tailwind-merge"],
		cookbook: [
			"Origin-aware animation",
			"Scale in",
			"Transform origin",
			"Enter / Exit",
			"Continuity transition",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/grow-dialog/GrowDialog.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "stiffness",
				kind: "number",
				default: 320,
				min: 100,
				max: 600,
				step: 10,
				description: "Spring stiffness of the grow.",
			},
			{
				name: "damping",
				kind: "number",
				default: 34,
				min: 10,
				max: 60,
				step: 1,
				description: "Spring damping — lower rings longer.",
			},
			{
				name: "radius",
				kind: "number",
				default: 16,
				min: 0,
				max: 32,
				step: 1,
				unit: "px",
				description: "Panel corner radius.",
			},
			{
				name: "blurBackdrop",
				kind: "boolean",
				default: true,
				description: "Frosted blur on the backdrop scrim.",
			},
			{
				name: "autoFocus",
				kind: "boolean",
				default: true,
				description: "Focus the first focusable element on open.",
			},
		],
	},
	{
		slug: "flip-card",
		name: "FlipCard",
		category: "Micro-interactions",
		tier: "pro",
		description:
			"A grid card that flips out of its cell — GSAP Flip carries it to viewport center while it rotates to reveal its back face, then returns on close.",
		dependencies: ["gsap", "lucide-react", "clsx", "tailwind-merge"],
		cookbook: [
			"3D tilt / Flip",
			"Shared element transition",
			"Layout animation",
			"Perspective",
			"Rotate",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/flip-card/FlipCard.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "flipDuration",
				label: "Flip duration",
				kind: "number",
				default: 0.5,
				min: 0.2,
				max: 1.2,
				step: 0.05,
				unit: "s",
				description:
					"Duration of the Flip flight; rotation timing derives from it.",
			},
			{
				name: "perspective",
				kind: "number",
				default: 1200,
				min: 400,
				max: 2400,
				step: 100,
				unit: "px",
				description: "3D perspective depth on each card.",
			},
			{
				name: "expandedWidth",
				label: "Expanded width",
				kind: "number",
				default: 400,
				min: 280,
				max: 640,
				step: 10,
				unit: "px",
				description: "Expanded card width (clamped to the viewport).",
			},
			{
				name: "expandedHeight",
				label: "Expanded height",
				kind: "number",
				default: 480,
				min: 320,
				max: 720,
				step: 10,
				unit: "px",
				description: "Expanded card height (clamped to the viewport).",
			},
			{
				name: "backdropBlur",
				label: "Backdrop blur",
				kind: "number",
				default: 2,
				min: 0,
				max: 16,
				step: 1,
				unit: "px",
				description: "Backdrop scrim blur while a card is open.",
			},
		],
	},
	{
		slug: "blueprint-card",
		name: "BlueprintCard",
		category: "Micro-interactions",
		tier: "free",
		description:
			"A technical-drawing card — dashed envelope, accent rules, corner nodes, and a soft glow that draw in on mount.",
		dependencies: ["clsx", "tailwind-merge"],
		cookbook: ["Line drawing", "Enter / Exit", "Stagger"],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/blueprint-card/BlueprintCard.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "accentColor",
				kind: "color",
				default: "#a855f7",
				description: "Accent color for the frame, nodes, and glow.",
			},
			{
				name: "dashed",
				kind: "boolean",
				default: true,
				description: "Dashed outer border; solid when off.",
			},
			{
				name: "dotSize",
				label: "Node size",
				kind: "number",
				default: 4,
				min: 2,
				max: 10,
				step: 1,
				unit: "px",
				description: "Diameter of the corner nodes.",
			},
			{
				name: "glowIntensity",
				label: "Glow intensity",
				kind: "number",
				default: 0.15,
				min: 0,
				max: 0.5,
				step: 0.01,
				description: "Strength of the bottom gradient glow.",
			},
			{
				name: "animated",
				kind: "boolean",
				default: true,
				description: "Draw the frame in on mount.",
			},
			{
				name: "duration",
				label: "Duration",
				kind: "number",
				default: 0.6,
				min: 0.2,
				max: 5,
				step: 0.05,
				unit: "s",
				description: "Draw-in animation duration.",
				disabledWhen: { prop: "animated", equals: false },
			},
		],
	},
	{
		slug: "corner-letters",
		name: "CornerLetters",
		category: "Micro-interactions",
		tier: "free",
		description:
			"Four glyphs pinned to the container corners that blur-and-scale in on mount and dissolve out on exit.",
		dependencies: ["motion"],
		cookbook: ["Enter / Exit", "Blur", "Scale in", "Stagger"],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/corner-letters/CornerLetters.tsx",
			},
		],
		props: [
			{
				name: "visible",
				kind: "boolean",
				default: true,
				description: "Mounts and unmounts the glyphs — toggle to replay.",
			},
			{
				name: "stiffness",
				kind: "number",
				default: 120,
				min: 40,
				max: 400,
				step: 5,
				description: "Spring stiffness of the entrance.",
			},
			{
				name: "blur",
				kind: "number",
				default: 8,
				min: 0,
				max: 20,
				step: 1,
				unit: "px",
				description: "Blur the glyphs travel from and to.",
			},
			{
				name: "scaleFrom",
				label: "Scale from",
				kind: "number",
				default: 1.5,
				min: 1,
				max: 3,
				step: 0.05,
				description: "Starting scale of each glyph.",
			},
			{
				name: "offset",
				kind: "number",
				default: 4,
				min: 0,
				max: 32,
				step: 1,
				unit: "px",
				description: "Inset from the corners.",
			},
			{
				name: "size",
				kind: "number",
				default: 32,
				min: 16,
				max: 64,
				step: 2,
				unit: "px",
				description: "Glyph font size.",
			},
			{
				name: "color",
				kind: "color",
				default: "#a855f7",
				description: "Glyph color.",
			},
		],
	},
	{
		slug: "hover-list",
		name: "HoverList",
		category: "Micro-interactions",
		tier: "pro",
		description:
			"A display-type list where a shared accent block springs between rows on hover while the text indents and inverts.",
		dependencies: ["motion", "clsx", "tailwind-merge"],
		cookbook: [
			"Hover effect",
			"Layout animation",
			"Spring",
			"Continuity transition",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/hover-list/HoverList.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "highlightColor",
				label: "Highlight",
				kind: "color",
				default: "#a855f7",
				description: "Color of the sliding highlight block.",
			},
			{
				name: "invertedColor",
				label: "Hovered text",
				kind: "color",
				default: "#000000",
				description: "Text and icon color while a row is highlighted.",
			},
			{
				name: "stiffness",
				kind: "number",
				default: 300,
				min: 100,
				max: 600,
				step: 10,
				description: "Spring stiffness of the sliding highlight.",
			},
			{
				name: "damping",
				kind: "number",
				default: 30,
				min: 10,
				max: 60,
				step: 1,
				description: "Spring damping — lower rings longer.",
			},
			{
				name: "indent",
				kind: "number",
				default: 32,
				min: 0,
				max: 80,
				step: 2,
				unit: "px",
				description: "How far the row content slides right on hover.",
			},
			{
				name: "textSize",
				label: "Text size",
				kind: "enum",
				default: "md",
				options: ["sm", "md", "lg"],
				description: "Display type scale of the rows.",
			},
			{
				name: "showIcon",
				label: "Crosshair",
				kind: "boolean",
				default: true,
				description: "Rotate a crosshair icon in on hover.",
			},
			{
				name: "rounded",
				kind: "number",
				default: 0,
				min: 0,
				max: 24,
				step: 1,
				unit: "px",
				description: "Corner radius of the highlight block.",
			},
		],
	},
	{
		slug: "preloader",
		name: "Preloader",
		category: "Micro-interactions",
		tier: "free",
		description:
			"A percentage-counting load overlay — spinning ring, eased counter, and a fade-out that reveals the page beneath.",
		dependencies: ["gsap", "clsx", "tailwind-merge"],
		cookbook: [
			"Loop",
			"Number ticker",
			"Fade in / Fade out",
			"Easing",
			"Perceived performance",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/preloader/Preloader.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "duration",
				kind: "number",
				default: 3,
				min: 1,
				max: 8,
				step: 0.1,
				unit: "s",
				description: "Simulated load time.",
			},
			{
				name: "ringColor",
				label: "Ring color",
				kind: "color",
				default: "#a855f7",
				description: "Color of the spinner, counter, and label.",
			},
			{
				name: "overlayColor",
				label: "Overlay",
				kind: "color",
				default: "#000000",
				description: "Overlay background color.",
			},
			{
				name: "ringSize",
				label: "Ring size",
				kind: "number",
				default: 128,
				min: 64,
				max: 200,
				step: 4,
				unit: "px",
				description: "Ring diameter.",
			},
			{
				name: "spinSpeed",
				label: "Spin speed",
				kind: "number",
				default: 1.5,
				min: 0.5,
				max: 4,
				step: 0.1,
				unit: "s",
				description: "Seconds per spinner revolution.",
			},
			{
				name: "holdDelay",
				label: "Hold at 100%",
				kind: "number",
				default: 0.4,
				min: 0,
				max: 2,
				step: 0.1,
				unit: "s",
				description: "Pause at 100% before the fade.",
			},
			{
				name: "fadeDuration",
				label: "Fade duration",
				kind: "number",
				default: 0.6,
				min: 0.2,
				max: 1.5,
				step: 0.05,
				unit: "s",
				description: "Duration of the fade-out.",
			},
			{
				name: "showLabel",
				label: "Label",
				kind: "boolean",
				default: true,
				description: "Show the caption under the counter.",
			},
		],
	},
	{
		slug: "circle-menu",
		name: "CircleMenu",
		category: "Micro-interactions",
		tier: "free",
		description:
			"A menu overlay that blooms open as a clip-path circle spring while items stagger in with blur and rotation, then collapses back to its pill.",
		dependencies: ["motion", "clsx", "tailwind-merge"],
		cookbook: [
			"Clip-path",
			"Reveal",
			"Spring",
			"Stagger",
			"Blur",
			"Rotate",
			"Enter / Exit",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/circle-menu/CircleMenu.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "openStiffness",
				label: "Open spring",
				kind: "number",
				default: 20,
				min: 5,
				max: 120,
				step: 1,
				description: "Bloom spring stiffness — low values settle slowly.",
			},
			{
				name: "stagger",
				kind: "number",
				default: 0.09,
				min: 0,
				max: 0.2,
				step: 0.01,
				unit: "s",
				description: "Delay between each item's entrance.",
			},
			{
				name: "blur",
				label: "Item blur",
				kind: "number",
				default: 8,
				min: 0,
				max: 20,
				step: 1,
				unit: "px",
				description: "Blur the items travel from and to.",
			},
			{
				name: "overlayColor",
				label: "Overlay",
				kind: "color",
				default: "#0a0a0c",
				description: "Overlay background color.",
			},
			{
				name: "overlayOpacity",
				label: "Overlay opacity",
				kind: "number",
				default: 0.92,
				min: 0.5,
				max: 1,
				step: 0.01,
				description: "Overlay background opacity.",
			},
		],
	},
	{
		slug: "magnetic-button",
		name: "MagneticButton",
		category: "Micro-interactions",
		tier: "pro",
		description:
			"A button that leans into your cursor — caught in a magnetic field it slides toward the pointer while an amethyst aura swells with proximity, then snaps home on a spring and answers every click with a dark ripple bursting outward from the point of contact.",
		dependencies: ["motion"],
		cookbook: [
			"Hover effect",
			"Spring",
			"Ripple",
			"Press / Tap feedback",
			"Momentum",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/magnetic-button/MagneticButton.tsx",
			},
		],
		props: [
			{
				name: "pullStrength",
				label: "Pull",
				kind: "number",
				default: 0.4,
				min: 0,
				max: 1,
				step: 0.05,
				description: "Fraction of the cursor offset the button follows.",
			},
			{
				name: "radius",
				kind: "number",
				default: 120,
				min: 40,
				max: 300,
				step: 10,
				unit: "px",
				description: "Field radius where the magnetic pull begins.",
			},
			{
				name: "stiffness",
				kind: "number",
				default: 260,
				min: 100,
				max: 600,
				step: 10,
				description: "Spring stiffness of the pull and return.",
			},
			{
				name: "damping",
				kind: "number",
				default: 18,
				min: 8,
				max: 60,
				step: 1,
				description: "Spring damping — lower overshoots and rings.",
			},
			{
				name: "auraColor",
				label: "Aura",
				kind: "color",
				default: "#a855f7",
				description: "Color of the proximity glow.",
			},
			{
				name: "auraSize",
				label: "Aura size",
				kind: "number",
				default: 80,
				min: 0,
				max: 200,
				step: 5,
				unit: "px",
				description: "Spread of the aura at full proximity.",
			},
			{
				name: "ripple",
				kind: "boolean",
				default: true,
				description: "Emit a dark ripple burst on click.",
			},
			{
				name: "rippleColor",
				label: "Ripple",
				kind: "color",
				default: "#d2abfd",
				description: "Color of the click ripple.",
				disabledWhen: { prop: "ripple", equals: false },
			},
		],
	},
	{
		slug: "shadow-cursor",
		name: "ShadowCursor",
		category: "Micro-interactions",
		tier: "pro",
		description:
			"A custom cursor with a shadow at its heel — a precise dot leads while a lagging blade trails on a spring, and when it crosses an interactive element the trail pools and swells around it, inverting whatever lies beneath. Scoped entirely to its own stage; the page cursor is never touched.",
		dependencies: ["motion"],
		cookbook: ["Spring", "Follow-through", "Hover effect", "Momentum"],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/shadow-cursor/ShadowCursor.tsx",
			},
		],
		props: [
			{
				name: "dotSize",
				label: "Dot",
				kind: "number",
				default: 8,
				min: 4,
				max: 24,
				step: 1,
				unit: "px",
				description: "Size of the leading precise dot.",
			},
			{
				name: "trailSize",
				label: "Trail",
				kind: "number",
				default: 36,
				min: 12,
				max: 80,
				step: 2,
				unit: "px",
				description: "Size of the lagging trailing ring.",
			},
			{
				name: "stiffness",
				kind: "number",
				default: 150,
				min: 50,
				max: 500,
				step: 10,
				description:
					"Spring stiffness of the trailing ring — higher lags less.",
			},
			{
				name: "damping",
				kind: "number",
				default: 15,
				min: 8,
				max: 40,
				step: 1,
				description: "Spring damping of the trailing ring.",
			},
			{
				name: "cursorColor",
				label: "Color",
				kind: "color",
				default: "#a855f7",
				description: "Color of the dot and trailing ring.",
			},
			{
				name: "blendMode",
				label: "Blend",
				kind: "enum",
				default: "difference",
				options: ["difference", "screen", "exclusion", "normal"],
				description: "Mix-blend-mode of the cursor over content beneath it.",
			},
			{
				name: "poolScale",
				label: "Pool",
				kind: "number",
				default: 2,
				min: 1,
				max: 4,
				step: 0.1,
				unit: "×",
				description:
					"How much the trail swells when over an interactive element.",
			},
		],
	},
	{
		slug: "torchlight",
		name: "Torchlight",
		category: "Micro-interactions",
		tier: "pro",
		addedAt: "2026-07-24",
		description:
			"Content hidden in darkness; the cursor is a torch that reveals only what it touches.",
		dependencies: ["clsx", "tailwind-merge"],
		cookbook: ["Mask", "Reveal", "Hover effect", "Idle animation"],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/torchlight/Torchlight.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "radius",
				kind: "number",
				default: 140,
				min: 40,
				max: 400,
				step: 10,
				unit: "px",
				description: "Torch light radius.",
			},
			{
				name: "softness",
				kind: "number",
				default: 0.5,
				min: 0,
				max: 1,
				step: 0.05,
				description: "Falloff softness of the light edge.",
			},
			{
				name: "darkness",
				kind: "number",
				default: 0.92,
				min: 0.5,
				max: 1,
				step: 0.01,
				description: "How hidden the surrounding content is.",
			},
			{
				name: "restReveal",
				kind: "number",
				default: 0,
				min: 0,
				max: 0.3,
				step: 0.01,
				description: "Ambient reveal away from the torch.",
			},
			{
				name: "smoothing",
				kind: "number",
				default: 0.15,
				min: 0.02,
				max: 1,
				step: 0.01,
				description: "How much the light lags the cursor.",
			},
			{
				name: "flicker",
				kind: "number",
				default: 0.15,
				min: 0,
				max: 1,
				step: 0.05,
				description: "Subtle torch flicker (off under reduced motion).",
			},
			{
				name: "torchColor",
				kind: "color",
				default: "#a855f7",
				description: "Amethyst torch tint.",
			},
		],
	},
	{
		slug: "dissolve",
		name: "Dissolve",
		category: "Micro-interactions",
		tier: "pro",
		addedAt: "2026-07-24",
		description:
			"An element disintegrates into drifting shadow-motes on dismiss, and reforms on command.",
		dependencies: ["gsap", "clsx", "tailwind-merge"],
		cookbook: ["Enter / Exit", "Stagger", "Reveal"],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/dissolve/Dissolve.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "dismissed",
				kind: "boolean",
				default: false,
				description: "Toggle to disintegrate; toggle back to reform.",
			},
			{
				name: "particleGap",
				kind: "number",
				default: 4,
				min: 2,
				max: 12,
				step: 1,
				unit: "px",
				description: "Sampling stride; lower is finer motes.",
			},
			{
				name: "drift",
				kind: "number",
				default: 60,
				min: 10,
				max: 200,
				step: 5,
				unit: "px",
				description: "How far the motes scatter.",
			},
			{
				name: "duration",
				kind: "number",
				default: 1,
				min: 0.3,
				max: 3,
				step: 0.1,
				unit: "s",
				description: "Length of the dissolve.",
			},
			{
				name: "stagger",
				kind: "number",
				default: 0.4,
				min: 0,
				max: 1.5,
				step: 0.05,
				unit: "s",
				description: "Spread of per-mote start times.",
			},
			{
				name: "gravity",
				kind: "number",
				default: -0.4,
				min: -2,
				max: 2,
				step: 0.1,
				description: "Vertical bias; negative rises like ash.",
			},
			{
				name: "swirl",
				kind: "number",
				default: 0.5,
				min: 0,
				max: 2,
				step: 0.05,
				description: "Turbulent curl on the mote paths.",
			},
			{
				name: "reformOnTrigger",
				kind: "boolean",
				default: true,
				description: "Reassemble from motes when re-triggered.",
			},
			{
				name: "moteColor",
				kind: "color",
				default: "#a855f7",
				description: "Amethyst tint blended into the shadow-motes.",
			},
		],
	},
	{
		slug: "tilt",
		name: "Tilt",
		category: "Micro-interactions",
		tier: "free",
		addedAt: "2026-07-26",
		description:
			"A machined card that leans toward the pointer in 3D while a specular sheet slides across its face.",
		dependencies: ["clsx", "tailwind-merge"],
		cookbook: [
			"3D tilt / Flip",
			"Perspective",
			"Transform origin",
			"Hover effect",
			"Asymmetric easing",
			"Hardware acceleration",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/tilt/Tilt.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "maxTilt",
				kind: "number",
				default: 10,
				min: 0,
				max: 25,
				step: 1,
				unit: "°",
				description: "Maximum lean at the card's edge.",
			},
			{
				name: "perspective",
				kind: "number",
				default: 900,
				min: 300,
				max: 2000,
				step: 50,
				unit: "px",
				description: "Depth of the 3D projection — lower exaggerates it.",
			},
			{
				name: "glare",
				kind: "number",
				default: 0.35,
				min: 0,
				max: 1,
				step: 0.05,
				description: "Opacity of the specular sheet, matte → mirror.",
			},
			{
				name: "glareSize",
				kind: "number",
				default: 0.9,
				min: 0.3,
				max: 2,
				step: 0.05,
				description: "Glare radius as a fraction of card width.",
			},
			{
				name: "hoverScale",
				kind: "number",
				default: 1.03,
				min: 1,
				max: 1.15,
				step: 0.01,
				description: "Scale applied while the pointer is over the card.",
			},
			{
				name: "smoothing",
				kind: "number",
				default: 0.28,
				min: 0.05,
				max: 1,
				step: 0.01,
				unit: "s",
				description: "How long the card takes to catch up to the pointer.",
			},
			{
				name: "glareColor",
				kind: "color",
				default: "#c4b5fd",
				description: "Tint of the specular sheet.",
			},
		],
	},
	{
		slug: "sigil",
		name: "Sigil",
		category: "Micro-interactions",
		tier: "free",
		addedAt: "2026-07-26",
		description:
			"An SVG crest that traces itself in when it scrolls into view, stroke by stroke.",
		dependencies: ["clsx", "tailwind-merge"],
		cookbook: [
			"Line drawing",
			"Reveal",
			"Scroll reveal",
			"Enter / Exit",
			"Delay",
			"Duration",
			"Fill mode",
			"Stagger",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/sigil/Sigil.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "duration",
				kind: "number",
				default: 1800,
				min: 400,
				max: 5000,
				step: 100,
				unit: "ms",
				description: "Time for a single stroke to draw.",
			},
			{
				name: "delay",
				kind: "number",
				default: 0,
				min: 0,
				max: 2000,
				step: 50,
				unit: "ms",
				description: "Wait after entering view before the first stroke.",
			},
			{
				name: "strokeWidth",
				kind: "number",
				default: 1.5,
				min: 0.5,
				max: 6,
				step: 0.5,
				description: "Stroke width in viewBox units.",
			},
			{
				name: "glow",
				kind: "number",
				default: 0.5,
				min: 0,
				max: 1,
				step: 0.05,
				description: "Bloom trailing the stroke.",
			},
			{
				name: "replayOnLeave",
				kind: "boolean",
				default: true,
				description: "Re-arm on exit so the crest redraws on the way back.",
			},
			{
				name: "easing",
				kind: "enum",
				default: "ease-out",
				options: ["ease-out", "linear", "ease-in-out", "dramatic"],
				description: "Curve the stroke advances on.",
			},
			{
				name: "strokeColor",
				kind: "color",
				default: "#a855f7",
				description: "Amethyst stroke.",
			},
		],
	},
	{
		slug: "rift",
		name: "Rift",
		category: "Micro-interactions",
		tier: "pro",
		addedAt: "2026-07-26",
		description:
			"A draggable tear between two overlaid scenes — push past an edge and it resists, then snaps back.",
		dependencies: ["clsx", "tailwind-merge"],
		cookbook: [
			"Clip-path",
			"Before / after slider",
			"Drag",
			"Rubber-banding",
			"Continuity transition",
			"Reveal",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/rift/Rift.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "position",
				kind: "number",
				default: 50,
				min: 0,
				max: 100,
				step: 1,
				unit: "%",
				description: "Resting position of the tear.",
			},
			{
				name: "handleWidth",
				kind: "number",
				default: 2,
				min: 1,
				max: 8,
				step: 1,
				unit: "px",
				description: "Thickness of the divider line.",
			},
			{
				name: "rubberBand",
				kind: "number",
				default: 0.35,
				min: 0,
				max: 1,
				step: 0.05,
				description: "Give when dragged past an edge, hard stop → loose.",
			},
			{
				name: "angle",
				kind: "number",
				default: 0,
				min: -20,
				max: 20,
				step: 1,
				unit: "°",
				description: "Lean of the tear off vertical.",
			},
			{
				name: "snapBack",
				kind: "boolean",
				default: false,
				description: "Return to the resting position on release.",
			},
			{
				name: "glow",
				kind: "number",
				default: 0.4,
				min: 0,
				max: 1,
				step: 0.05,
				description: "Bloom along the divider.",
			},
			{
				name: "accentColor",
				kind: "color",
				default: "#a855f7",
				description: "Amethyst divider.",
			},
		],
	},
	{
		slug: "unfold",
		name: "Unfold",
		category: "Micro-interactions",
		tier: "pro",
		addedAt: "2026-07-26",
		description:
			"A row of slats that open on approach while their neighbours give up the room on a spring.",
		dependencies: ["motion", "clsx", "tailwind-merge"],
		cookbook: [
			"Accordion / Collapse",
			"Layout animation",
			"Spring",
			"Stiffness / Tension",
			"Damping",
			"Hover effect",
			"Layout thrashing",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/unfold/Unfold.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "panels",
				kind: "number",
				default: 5,
				min: 2,
				max: 7,
				step: 1,
				description: "How many slats to render.",
			},
			{
				name: "expandRatio",
				kind: "number",
				default: 3.5,
				min: 1.5,
				max: 8,
				step: 0.5,
				unit: "×",
				description: "How much wider the open slat is than a closed one.",
			},
			{
				name: "stiffness",
				kind: "number",
				default: 260,
				min: 60,
				max: 500,
				step: 20,
				description: "Spring stiffness — higher snaps open faster.",
			},
			{
				name: "damping",
				kind: "number",
				default: 30,
				min: 8,
				max: 60,
				step: 2,
				description: "Spring damping — lower overshoots and settles.",
			},
			{
				name: "labelReveal",
				kind: "boolean",
				default: true,
				description: "Turn the vertical rail label horizontal on open.",
			},
			{
				name: "useImage",
				kind: "boolean",
				default: false,
				description: "Fade each panel's cut-out artwork in behind its slat.",
			},
			{
				name: "gap",
				kind: "number",
				default: 8,
				min: 0,
				max: 32,
				step: 2,
				unit: "px",
				description: "Space between slats.",
			},
		],
	},
	{
		slug: "approach",
		name: "Approach",
		category: "Micro-interactions",
		tier: "pro",
		addedAt: "2026-07-26",
		description:
			"A tile whose overlay enters from the exact edge your cursor crossed, and leaves the way you go.",
		dependencies: ["clsx", "tailwind-merge"],
		cookbook: [
			"Direction-aware transition",
			"Enter / Exit",
			"Translate",
			"Skew",
			"Hover effect",
			"Spatial consistency",
			"Cubic-bezier",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/approach/Approach.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "overlayOpacity",
				kind: "number",
				default: 0.92,
				min: 0.2,
				max: 1,
				step: 0.02,
				description: "Opacity of the overlay panel.",
			},
			{
				name: "duration",
				kind: "number",
				default: 420,
				min: 120,
				max: 1200,
				step: 20,
				unit: "ms",
				description: "Travel time of the overlay.",
			},
			{
				name: "skew",
				kind: "number",
				default: 4,
				min: 0,
				max: 16,
				step: 1,
				unit: "°",
				description: "Shear while the overlay is off-position.",
			},
			{
				name: "blur",
				kind: "number",
				default: 0,
				min: 0,
				max: 12,
				step: 1,
				unit: "px",
				description: "Blur on the overlay while it is moving.",
			},
			{
				name: "offsetDistance",
				kind: "number",
				default: 100,
				min: 40,
				max: 160,
				step: 5,
				unit: "%",
				description: "How far off-tile the overlay starts.",
			},
			{
				name: "overlayColor",
				kind: "color",
				default: "#a855f7",
				description: "Amethyst overlay panel.",
			},
		],
	},
	{
		slug: "hold",
		name: "Hold",
		category: "Micro-interactions",
		tier: "free",
		addedAt: "2026-07-31",
		description:
			"A confirm that has to be earned — the ring fills while you hold, snaps shut when it commits, and empties on a different curve entirely if you let go early.",
		dependencies: ["motion", "clsx", "tailwind-merge"],
		cookbook: [
			"Hold to confirm",
			"Asymmetric easing",
			"Press / Tap feedback",
			"Interruptible animation",
			"Ease-out",
			"Purposeful animation",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/hold/Hold.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "duration",
				label: "Hold time",
				kind: "number",
				default: 1200,
				min: 300,
				max: 4000,
				step: 50,
				unit: "ms",
				description:
					"How long a full press has to be held before it commits. Under about 600ms people let go by reflex; past 2s they assume it is broken.",
			},
			{
				name: "indicator",
				label: "Indicator",
				kind: "enum",
				default: "ring",
				options: ["ring", "bar", "fill"],
				description:
					"Where the progress lives — a ring traced around the glyph, a bar under the control, or the control's own face flooding left to right.",
			},
			{
				name: "threshold",
				label: "Commit at",
				kind: "number",
				default: 0.92,
				min: 0.6,
				max: 1,
				step: 0.01,
				description:
					"Fraction of the track that counts as committed. Below 1 the last sliver closes itself, so the press lands a beat before the eye expects it — that early snap is what makes it feel decisive rather than punctual.",
			},
			{
				name: "rewindRate",
				label: "Rewind rate",
				kind: "number",
				default: 2.4,
				min: 0.5,
				max: 6,
				step: 0.1,
				unit: "×",
				description:
					"How much faster a cancelled hold empties than it filled. At 1 it rewinds the same tape; above 2 the release reads as a spring letting go, which is the entire point of an asymmetric curve.",
			},
			{
				name: "strokeWidth",
				label: "Track weight",
				kind: "number",
				default: 3,
				min: 1,
				max: 12,
				step: 0.5,
				unit: "px",
				disabledWhen: { prop: "indicator", equals: "fill" },
				description:
					"Weight of the progress track. Thin reads as instrumentation, thick reads as a toy.",
			},
			{
				name: "holdScale",
				label: "Press sink",
				kind: "number",
				default: 0.96,
				min: 0.85,
				max: 1,
				step: 0.01,
				unit: "×",
				description:
					"How far the control sinks under the finger. The press feedback is doing more work here than the ring is — take it to 1 and the hold stops feeling physical.",
			},
			{
				name: "commitScale",
				label: "Commit pop",
				kind: "number",
				default: 1.06,
				min: 1,
				max: 1.3,
				step: 0.01,
				unit: "×",
				description:
					"Overshoot the control pops to at the instant of commit, before settling. This is the receipt.",
			},
			{
				name: "successHold",
				label: "Confirmed for",
				kind: "number",
				default: 1.4,
				min: 0.4,
				max: 4,
				step: 0.2,
				unit: "s",
				description:
					"How long the confirmed state stays on screen before the control re-arms itself.",
			},
			{
				name: "accentColor",
				label: "Accent",
				kind: "color",
				default: "#a855f7",
				description: "Amethyst of the filling track and the confirmed state.",
			},
			{
				name: "trackColor",
				label: "Track",
				kind: "color",
				default: "#2a2136",
				description:
					"The unfilled remainder. Keep it close to the surface underneath — a high-contrast track turns the ring into a bullseye.",
			},
		],
	},
	{
		slug: "dismiss",
		name: "Dismiss",
		category: "Micro-interactions",
		tier: "free",
		addedAt: "2026-07-31",
		description:
			"Throw the top card away and the stack closes up behind it. Distance commits, but so does speed — a fast flick from barely anywhere counts, and anything short of both rubber-bands home carrying the velocity you let go at.",
		dependencies: ["motion", "clsx", "tailwind-merge"],
		cookbook: [
			"Swipe to dismiss",
			"Rubber-banding",
			"Velocity",
			"Momentum",
			"Interruptible animation",
			"Drag",
			"Spring",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/dismiss/Dismiss.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "axis",
				label: "Throw axis",
				kind: "enum",
				default: "x",
				options: ["x", "y", "both"],
				description:
					"Which way a card can leave. Sideways is the toast convention; downward reads as filing something away rather than discarding it.",
			},
			{
				name: "threshold",
				label: "Commit distance",
				kind: "number",
				default: 32,
				min: 10,
				max: 90,
				step: 1,
				unit: "%",
				description:
					"How far across its own width a card must travel to commit. Distance is only half the test — a flick past the speed threshold commits from anywhere.",
			},
			{
				name: "velocityThreshold",
				label: "Commit speed",
				kind: "number",
				default: 520,
				min: 100,
				max: 2000,
				step: 20,
				unit: "px/s",
				description:
					"Release speed that commits regardless of how far the card actually moved. Without this a quick flick just springs back and the gesture feels deaf.",
			},
			{
				name: "rubberBand",
				label: "Rubber band",
				kind: "number",
				default: 0.35,
				min: 0,
				max: 1,
				step: 0.05,
				description:
					"Give past the drag bounds. 0 is a hard wall; 1 follows the finger exactly and loses the sense that the card is resisting you.",
			},
			{
				name: "stackDepth",
				label: "Cards shown",
				kind: "number",
				default: 3,
				min: 1,
				max: 6,
				step: 1,
				description:
					"How many cards are visible at once. Past four the ones at the back stop being legible and start being texture.",
			},
			{
				name: "stackOffset",
				label: "Stack step",
				kind: "number",
				default: 14,
				min: 0,
				max: 40,
				step: 1,
				unit: "px",
				description:
					"Vertical step between one card and the next down the stack.",
			},
			{
				name: "stackScale",
				label: "Stack taper",
				kind: "number",
				default: 0.05,
				min: 0,
				max: 0.2,
				step: 0.01,
				description:
					"Scale lost per card down the stack. This is what sells depth — offset alone reads as a list.",
			},
			{
				name: "rotateOnDrag",
				label: "Lean",
				kind: "number",
				default: 8,
				min: 0,
				max: 30,
				step: 1,
				unit: "°",
				description:
					"Degrees the card leans at the far edge of the throw. Derived from displacement, not animated separately, so an interrupted throw can never desync it.",
			},
			{
				name: "stiffness",
				label: "Stiffness",
				kind: "number",
				default: 420,
				min: 80,
				max: 900,
				step: 10,
				description:
					"How hard the stack springs into its new slot after a card leaves.",
			},
			{
				name: "damping",
				label: "Damping",
				kind: "number",
				default: 32,
				min: 8,
				max: 80,
				step: 1,
				description:
					"How fast that spring settles. Low values let the stack overshoot and bounce, which reads as lightweight.",
			},
		],
	},
	{
		slug: "aperture",
		name: "Aperture",
		category: "Micro-interactions",
		tier: "pro",
		addedAt: "2026-07-31",
		description:
			"A camera iris. Blades pivot on a ring of pins and one shared angle drives the whole mechanism — wide open at 64% of the housing, past the centre and light-tight at zero, cutting the scalloped polygon a real iris makes on the way through.",
		dependencies: ["motion", "clsx", "tailwind-merge"],
		cookbook: [
			"Reveal",
			"Rotate",
			"Clip-path",
			"Orchestration",
			"Bounce",
			"Ease-in-out",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/aperture/Aperture.tsx",
			},
			UTILS_VARIANT,
		],
		props: [
			{
				name: "bladeCount",
				label: "Blades",
				kind: "number",
				default: 8,
				min: 3,
				max: 12,
				step: 1,
				description:
					"How many leaves in the mechanism. Low counts give the hard hexagonal bokeh of a cheap lens; past nine the opening is indistinguishable from a circle, which is exactly what expensive glass is sold on.",
			},
			{
				name: "openness",
				label: "Openness",
				kind: "number",
				default: 0.62,
				min: 0,
				max: 1,
				step: 0.01,
				disabledWhen: { prop: "autoOpen", equals: true },
				description:
					"Where the blades rest, 0 shut to 1 wide. At 0 they cross past the centre rather than merely meeting it — an iris that only just touches leaks light.",
			},
			{
				name: "duration",
				label: "Travel",
				kind: "number",
				default: 900,
				min: 150,
				max: 2500,
				step: 50,
				unit: "ms",
				description: "Time for one full open or close.",
			},
			{
				name: "detent",
				label: "Detent",
				kind: "number",
				default: 0.35,
				min: 0,
				max: 1,
				step: 0.05,
				description:
					"Overshoot as the mechanism seats. 0 arrives dead, which reads as software; a little bounce reads as a part with mass hitting its stop.",
			},
			{
				name: "bladeCurvature",
				label: "Blade curve",
				kind: "number",
				default: 1,
				min: 0.6,
				max: 1.4,
				step: 0.02,
				unit: "×",
				description:
					"Curvature of each blade's inner edge. The mechanism is derived from the opening range, so this bends the leaf without touching the travel: flatter blades cut a rounder, more regular polygon, tighter ones pull the scallop corners out into a star.",
			},
			{
				name: "edgeHighlight",
				label: "Edge light",
				kind: "number",
				default: 0.6,
				min: 0,
				max: 1,
				step: 0.05,
				description:
					"Strength of the seam along each leaf's inner edge, and of the bezel. Each seam is cut by the leaf that lies over it — named rather than left to the drawing order, because a leaf stack is a ring and painter's order is a line. That is what puts exactly one line at every corner of the opening, wide open and shut alike.",
			},
			{
				name: "housingInset",
				label: "Bezel",
				kind: "number",
				default: 6,
				min: 0,
				max: 24,
				step: 1,
				description:
					"Width of the machined ring inside the rim. It is drawn over the blades, so the housing reads as the part they are seated in rather than one more leaf.",
			},
			{
				name: "autoOpen",
				label: "Cycle",
				kind: "boolean",
				default: true,
				description:
					"Open and shut on a loop. Turning it off hands the mechanism to the openness slider.",
			},
			{
				name: "bladeColor",
				label: "Blade",
				kind: "color",
				default: "#26232c",
				description:
					"Body of the blades, the housing, and the barrel floor behind them. Its own material rather than a themed surface — a light theme must not put a white ring around a lens.",
			},
			{
				name: "edgeColor",
				label: "Edge",
				kind: "color",
				default: "#000000",
				description: "Amethyst on the blade seams and the bezel.",
			},
		],
	},
	{
		slug: "intro-animation",
		name: "IntroAnimation",
		category: "Micro-interactions",
		tier: "pro",
		addedAt: "2026-08-01",
		pausable: true,
		description:
			"A wind front sweeps the wordmark right to left. The instant it reaches a glyph the text hands off to a baseline-aligned field of GPU grain — same pixels, same colour, no seam — which peels crown-first and drifts away as smoke through a curl field. The backdrop opens behind it to reveal the page. Mount it over your app and unmount on onDone.",
		dependencies: ["ogl"],
		cookbook: [
			"Reveal",
			"Enter / Exit",
			"Stagger",
			"Page transition",
			"Perceived performance",
			"Hardware acceleration",
			"Linear",
			"Reduced motion",
		],
		variants: [
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/intro-animation/IntroAnimation.tsx",
			},
			{
				lang: "ts",
				style: "tailwind",
				file: "components/registry/intro-animation/dust-gl.ts",
				role: "peer",
				label: "dust-gl.ts",
			},
			{
				lang: "ts",
				style: "tailwind",
				file: "hooks/use-animation-loop.ts",
				role: "hook",
				label: "use-animation-loop.ts",
			},
		],
		props: [
			{
				name: "sweepDuration",
				label: "Sweep",
				kind: "number",
				default: 1.9,
				min: 0.6,
				max: 5,
				step: 0.1,
				unit: "s",
				description:
					"Time the wind front takes to cross the wordmark. Everything downstream is derived from it — the reveal fires at 93.7% of the sweep, so shortening it tightens the whole sequence rather than leaving the plume stranded after a fast wipe.",
			},
			{
				name: "grainDensity",
				label: "Grain density",
				kind: "number",
				default: 2500,
				min: 400,
				max: 6000,
				step: 100,
				description:
					"Grains sampled per glyph. The sampling stride adapts to font size, so this is a count and not a spacing — a glyph holds roughly this many points whether it renders at 28px or 112px. The performance knob: every grain is one point in a single draw call, and changing it re-rasterizes the word.",
			},
			{
				name: "windSpeed",
				label: "Wind",
				kind: "number",
				default: 1,
				min: 0,
				max: 3,
				step: 0.05,
				unit: "×",
				description:
					"How hard the wind carries released grains downstream. Each grain keeps its own speed rank, so the motion-blur ghosting on the fastest ones survives turning the wind down — the plume slows without flattening into a uniform sheet.",
			},
			{
				name: "buoyancy",
				label: "Buoyancy",
				kind: "number",
				default: 1,
				min: 0,
				max: 3,
				step: 0.05,
				unit: "×",
				description:
					"How fast the plume climbs. Lift also feeds the wind shear, so raising it does not just push the dust up — it rakes the risen grains further ahead and bends the plume into a curve instead of a diagonal.",
			},
			{
				name: "turbulence",
				label: "Turbulence",
				kind: "number",
				default: 1,
				min: 0,
				max: 3,
				step: 0.05,
				unit: "×",
				description:
					"Amplitude of the divergence-free curl field the grains are advected through. The field is sampled at each grain's travelling position, so grains meet new eddies as they fly — that is the difference between smoke and shimmer. At zero the dust is pure ballistics.",
			},
			{
				name: "grainSize",
				label: "Grain size",
				kind: "number",
				default: 1,
				min: 0.4,
				max: 2.5,
				step: 0.05,
				unit: "×",
				description:
					"Size of an individual grain. The base covers the sampling stride so the held letter reads as solid ink; push it far below 1 and the handoff starts to show the lattice underneath.",
			},
			{
				name: "splitIndex",
				label: "Accent from",
				kind: "number",
				default: 6,
				min: 0,
				max: 16,
				step: 1,
				description:
					"How many characters carry the ink colour before the accent takes over. The tint travels with each grain, so the dust keeps the colour of the letter it came from all the way out.",
			},
			{
				name: "reveal",
				label: "Reveal",
				kind: "enum",
				default: "split",
				options: ["split", "fade", "none"],
				description:
					"How the backdrop leaves. Split slides two halves apart from the centre line; fade dissolves it in place; none draws no backdrop at all, so the wordmark erodes over whatever is already behind it.",
			},
			{
				name: "autoplay",
				label: "Autoplay",
				kind: "boolean",
				default: true,
				description:
					"Replay the sequence on a loop. Off is the real thing: the intro plays once, calls onDone, and returns false to the loop host so it stops costing frames for good.",
			},
			{
				name: "replayDelay",
				label: "Replay after",
				kind: "number",
				default: 1.2,
				min: 0,
				max: 6,
				step: 0.1,
				unit: "s",
				disabledWhen: { prop: "autoplay", equals: false },
				description:
					"Pause between the plume clearing and the wordmark reassembling. Long enough to read as a fresh entrance rather than a stutter.",
			},
			{
				name: "inkColor",
				label: "Ink",
				kind: "color",
				default: "#e9e7ef",
				description:
					"Colour of the leading characters and the dust that comes off them.",
			},
			{
				name: "accentColor",
				label: "Accent",
				kind: "color",
				default: "#a855f7",
				description:
					"Amethyst on the characters from the split onward. A uniform, not baked into the grain buffer, so dragging it never re-rasterizes the word.",
			},
			{
				name: "backgroundColor",
				label: "Backdrop",
				kind: "color",
				default: "#08050e",
				description:
					"The field the wordmark sits on and the thing the reveal opens. Its own material rather than a themed surface — an intro that flashes white in a light theme defeats the point of covering the page.",
			},
		],
	},
];
